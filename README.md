...
生成された既定GCCツールチェーンは、ルートCMakeでGit管理する
`toolchains/arm-none-eabi.cmake` に接続します。明示したコンパイラーを優先し、
未指定ならPATH、続いて標準インストール先のSTM32CubeCLTを探索します。
標準Cヘッダー・`libc`・`libm`・`nano.specs`を確認し、GCCと関連ツールを同じディレクトリーに揃えて
絶対パスをキャッシュするため、IDEと端末のPATH差やCMake再構成で別のGCCへ戻りません。
不完全なコンパイラーを明示した場合は、別のものに勝手に切り替えず構成エラーにします。

`stdio.h`、`math.h`、`stdint.h`が見つからない場合は、標準ライブラリーを含まないGCCが
選択されていないか確認してください。STM32CubeCLTなどの完全な配布を明示できます。

```sh
cmake --preset Debug -DARM_GNU_TOOLCHAIN_BIN_DIR=/path/to/GNU-tools-for-STM32/bin
cmake --build --preset Debug
```

旧キャッシュからの移行やコンパイラー変更時は、CMake 3.24以上なら同じ構成コマンドに
`--fresh`を追加します（独自のCMakeオプションも再指定）。3.22/3.23では新しいビルドディレクトリーを使います。
CLionは変更後に「CMakeプロジェクトの再読み込み」を実行し、必要なら
「キャッシュをリセットしてプロジェクトを再読み込み」で旧解析モデルを更新してください。
制御パラメーターやヘッダーへのダミー定義追加で回避する必要はありません。
