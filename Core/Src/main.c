...
{
  motor_app_receive_feedback(hfdcan, interrupts);
}

void HAL_FDCAN_RxFifo1Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t interrupts)
{
  motor_app_receive_command(hfdcan, interrupts);
}

void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  motor_app_control_tick(htim);
}
/* USER CODE END 0 */
