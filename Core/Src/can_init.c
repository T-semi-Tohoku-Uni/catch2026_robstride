...
void inter_board_CAN_filter_init(FDCAN_FilterTypeDef *Hfdcan_Filter_Settings)
{
  ...
  Hfdcan_Filter_Settings->FilterID1 = 0x00;
  Hfdcan_Filter_Settings->FilterID2 = 0x00;
}
...
HAL_StatusTypeDef inter_board_CAN_RxTxSettings_init(FDCAN_TxHeaderTypeDef *Htxheader)
{
  ...
  inter_board_CAN_filter_init(&FDCAN_Filter_settings_robstride);
  inter_board_CAN_txheader_init(Htxheader);
  ...
  if (HAL_OK != HAL_FDCAN_ConfigGlobalFilter(&hfdcan1, FDCAN_ACCEPT_IN_RX_FIFO1, FDCAN_ACCEPT_IN_RX_FIFO1, FDCAN_FILTER_REMOTE, FDCAN_FILTER_REMOTE))
  {
		printf("config global filter is error\r\n");
    return HAL_ERROR;
	}
