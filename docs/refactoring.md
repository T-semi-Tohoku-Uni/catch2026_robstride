...
## 2026-09-10: 初期化応答確認と失敗時停止

RobStride全3台の初期化を最大15秒間並行して再試行する。各設定書き込み後の新しい応答と、Enable後の正常な稼働応答を待ち、全3台が新鮮な正常応答を保持している場合だけ周期制御を開始する。待機中は停止位置を保持し、運転開始後のEL05だけの再初期化は廃止した。

一時的な送信登録失敗や応答欠落は再試行する。faultは自動クリアせず、正常な停止応答へ戻れば期限内で再開する。最終失敗時は全4台へSTOPを間隔を空けて再送し、最大1秒間停止応答を待ってからエラー停止する。通信不能の軸は停止未確認として記録する。

`motor_app_test`は実際のアプリ、RobStrideドライバー、CANデコーダーを使用し、HALとCyberGearの起動処理を模擬する。既存2件に19件を追加した。遅延起動、送信エラー、送信登録後の未到達、無応答・誤ID・誤モード・古い応答、途中faultと解消、STOPの再送・永続失敗、tick周回、ADRC・TIM開始失敗を検証する。全RobStride確認後、ADRC起動中に発生するfaultの再試行と、再試行で15秒の期限を延長しないことも確認する。

検証結果：Arm GNU 13.3.1のDebug・Releaseビルド成功、MinGW GCC 15.2.0（`-Wall -Wextra -Werror`）で21/21ホストテスト成功。出力先は`build/review-Debug`、`build/review-Release`、`build/review-app-tests`。実機書き込み・駆動確認は未実施。

以下の「変更内容」と既存ビルド計測は当初のリファクタリングの記録。現在の起動仕様は[詳細仕様](firmware-spec.md)第6・8・11節を参照。
