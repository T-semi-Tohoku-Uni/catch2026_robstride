...
2026-09-10更新。ユーザー指示により停止距離の予測保護モデル全体をいったん省略した。計算・運転中の判定・起動検査・専用パラメーターを削除し、その他の保護、STOP処理、ADRC・軌道・慣性モデルを維持する。全軸起動・通信の統合修正は[統合修正と空試験](09_統合修正と空試験.md)を参照。
...
現在の共通設定は `CYBERGEAR_HARDWARE_CONFIRMED=1`、電流上限6 A、b0=1（許可範囲0.35～3）を持つ。停止距離予測による起動拒否は省略したが、確認フラグとその他の数値検査は維持する。フラグを0にした場合や必須値がNANの場合は、ホーミングを始める前に起動を拒否する。ホストテストの±2 rad、2 A等は架空のfixtureであり転記しない。
...
`hard_min/max`、温度/速度、電流上限、b0は2章の必須マクロから設定する。`hardware_confirmed`は数値の整合性検証とは別に要求される。

停止距離の予測計算、外側境界までの距離との比較、同じ式による起動前の整合性検査は行わない。専用の保証減速度・外向き加速度・輸送遅延・停止余裕は設定項目から削除した。角度境界、速度・電流・温度・通信・追従・拘束の保護は維持し、計画失敗・モデル情報失効も理由付きのSTOP要求へ遷移する。軌道用の加減速電流予算・A/J制約と電流スルー制限は引き続き使用する。

**STOPコマンドの送信だけでは能動制動を保証しない**。STOP要求・送信・応答・静止確認の処理は維持するが、CAN断、電源断、駆動停止後の停止位置は保証しない。実機のSTOP動作/外部停止方法が不明なため、架空の能動制動動作や未知のwatchdogレジスタを追加していない。
...
2026-09-09時点の初回改修ではRobStrideの値/軌道/周期/角度オフセット、上位CAN ID/DLC/endian、return待ち、全体監視/reset、ホーミングの幾何、共有CAN設定/GPIO/IRQは変更していない。指定された`stm32g4xx_hal_msp.c`も確認したが、共有設定なので変更不要と判断した。

2026-09-10の統合修正では、全軸起動・停止・個別監視、上位通信の数値変換と指令処理も対象にした。現在の範囲は[統合記録](09_統合修正と空試験.md)を参照。モードreadbackを必須にしたため、応答形式が異なる搭載FWでは起動が失敗し得る。実機ログで合わせる。

2026-09-09の初回改修時点ではコミットを作成していない。元に戻す場合は今回の `cybergear.c/.h`、mainのCG接続、CMakeの新規3ソース登録を作業開始HEADの該当差分に戻し、新規モジュールをビルド対象から外す。README/追加説明/テストは残しても駆動に影響しない。作業開始時から存在したdocsや他の人の後続変更を一括削除・resetしない。比較モードやgamma=0は旧実装へのロールバックとは異なる。
