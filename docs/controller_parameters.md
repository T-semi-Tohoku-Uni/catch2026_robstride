...
`CG_STATE_WAIT_RUN`（phase=5）では、ENABLE後に新しく受信した新鮮なRun応答（mode=2）で
制御を開始する。電流ゼロでは位置保持力がなく、静止速度を100 ms継続して満たす条件は
微動や速度のばらつきで成立しないため、この段階では静止継続を要求しない。
ADRC観測器はその応答の実測位置・実測速度から開始し、現在位置を保持する参照軌道へ引き継ぐ。
投入済みゼロ電流から既存の電流変化率制限で制御を開始する。
ENABLE前のSTOP確認・静止待ち、モード読戻し、新しいRun応答の鮮度、起動期限、過速度保護は維持する。

## 電流制限継続時のパラメーター復帰

CANバス状態・エラーカウンター・状態取得失敗による `fault=3` の自動停止判定は省略する。
バス状態とカウンターは既存の診断用に取得し、送信キュー投入失敗・受信鮮度切れの停止判定は維持する。

`Core/Inc/cybergear_config.h` の `CYBERGEAR_SATURATION_RECOVERY_ENABLED` と
`CYBERGEAR_TRACKING_RECOVERY_ENABLED` は既定で `1`。
電流モードで、電流の振幅/変化率制限が `CYBERGEAR_SATURATION_TIMEOUT_MS` 続くか、
参照軌道との誤差が閾値を超えて `CYBERGEAR_TRACKING_TIMEOUT_MS` 続くと、
停止せず制御器の `b0` を `b0_initial`、姿勢モデルの `b0` を `fixed_b0`、
外乱推定値 `z3` をゼロに戻す。初期値は現在の運転開始時に確定した設定値
（単体試験の上書き・UART設定を含む）を使う。wc・zeta・woなどは自動学習しておらず、その設定値を保持する。

復帰検出周期から、現在の指令電流や変化率制限にかかわらず0 Aを送信し、
`CYBERGEAR_RECOVERY_ZERO_MS`（既定200 ms）以上、制御周期の丸め幅内で保持する。
ISRでは待機せず毎周期ゼロ電流を送る。送信成功時だけ投入済み電流履歴をゼロに更新し、
ESOは実際に投入成功した指令を使って位置・速度推定を続ける。保持中はb0と外乱推定値を初期値に保ち、
補償電流・補償ランプをゼロにする。保持終了後は0 Aから通常の電流変化率制限を適用し、
外乱補償の待機・ランプと、姿勢スケジュールが有効ならb0の更新も再開する。
位置・速度推定、受信履歴、原点、最終目標、運転状態はリセットしない。
STOP・ENABLE・モード設定・ゼロ設定・原点探索・起動シーケンスは実行しない。

復帰開始時に準備済み軌道を破棄し、計画世代を更新して計算中の古い計画も無効にする。
ゼロ保持中は目標更新を受け付けるが、新しい計画の作成・適用と計画待ちタイムアウトを抑止する。
保持終了時、その周期の電流計算より前に最新の実測位置を基準とする保持軌道へ置き換える。
参照速度・加速度はゼロとし、観測器の位置・速度推定は維持する。
mainの既存プランナーでその位置から最新の最終目標へ再計画し、先行時間経過後に適用する。
待機中はその位置を保持する制御を行う。実測位置がソフト可動域外なら位置異常、計画失敗なら計画異常で停止する。
追従・飽和・停滞の監視時間と計画待ち期限は保持終了から数え直す。
保持中の誤差によってゼロ保持時間を延長しない。単体試験では保持中を到達扱いにせず、区間順序・区間期限を維持する。
復帰の合計回数は `CyberGearMotor.recovery_count` とログの `recover`、ゼロ保持中は `zero=1` で確認できる。
停滞が復帰開始と同時に成立した場合は停滞停止を優先する。実測位置/速度・温度・通信・数値異常、
手動停止は保持中も有効。各復帰設定を `0` にすると対応するfault=13/15で停止する。
内蔵PD比較モードは電流ゼロを保証できないため、この復帰を行わず停止する。
