...
2026-09-10に、RobStride全3台の初期化応答確認・再試行と、失敗時の全軸停止処理を追加した。
...
後続バイトは無視する。初期化は**直前の有効値が0で、今回の有効値が1**の場合だけ要求する。
運転開始後の復帰は、従来どおり直前の有効値から何らかの変化があれば要求する。

| 条件 | 動作 |
| --- | --- |
| 最初の有効フレーム | 比較基準として記録。初期化しない |
| 前回と同じ値 | 起動・復帰トリガーなし |
| `0 → 1`に変化、運転開始前 | `motor_init_requested=true`。メイン側で初期化へ進む |
| `0 → 1`以外の変化、運転開始前 | 初期化を要求せず、比較基準だけ更新 |
...
初回受信が1の場合や、`1 → 0`、`0 → 2`、`2 → 1`では初期化しない。
初期化処理中の追加の`0 → 1`は要求フラグを立てるだけで、初期化を繰り返さない。
...
flowchart TD
    ...
    B --> C[0x500の有効値が0から1へ変化するのを待つ]
    C --> E[CyberGearの応答確認]
    E --> F[CyberGear原点探索・STOP]
    F --> G[RobStride全3台を並行初期化・正常稼働応答を確認]
    G --> H[CyberGear電流モード・ADRC開始]
    H --> I[TIM6開始・運転中に設定]
    I --> J[監視・復帰解除・ログ]
    G -->|15秒以内にそろわない| K[全4台STOP再送・停止確認後エラー停止]
```

1. 標準出力のバッファリングを無効化し、HAL、80 MHzクロック、GPIO、USART2、FDCAN1、TIM6、FDCAN3を初期化する。
2. 全軸のID・送信ヘッダーを準備し、FDCAN1、FDCAN3のフィルター・受信割り込み通知を設定してバスを開始する。
3. `0x500`で要求されるまで10 ms間隔で待つ。この間、ホーミング、Enable、周期制御、共通3秒監視を行わない。
4. 共通最終受信時刻を現在時刻に設定する。起動中は各初期化処理の期限を使い、共通監視のMCUリセットで停止処理を飛ばさない。
...
9. RobStride全3台を並行してPPモードに設定し、各軸のEnable後の正常稼働応答を待つ。手順は第8節。
10. 同一スナップショットで全3台がonline、fault=0、mode=2、位置が有限、受信後100 ms未満であることを確認する。CyberGearの応答で代用しない。
11. CyberGearをSTOP→10 ms→current mode=3→10 ms→電流0→10 msと設定。ADRC状態をゼロ化し、onlineをfalseに戻し、Enableする。
12. ADRC起動中の待ち時間の後、全RobStrideの最新応答を再確認してからTIM6割り込みを開始し、`motors_running=true`とする。この間に応答が古くなるか異常になった場合はCyberGearを停止し、当初の15秒期限内でRobStrideの初期化をやり直す。初期化所要時間は全確認の成功後に出力する。
13. メインループに入る。EL05も起動完了前に検証済みなので、運転開始後の再初期化は行わない。

CyberGear初期化、原点探索、RobStride初期化、ADRC開始、TIM6開始、モーターCAN設定の失敗では、第11節の全軸停止処理を経て`Error_Handler`へ進む。上位CAN設定失敗はモーターCAN起動前なので直接`Error_Handler`へ進む。
停止後の自動再起動は行わない。再実行にはMCUリセットと新しい`0 → 1`要求が必要。
...
初期化の期限は3台共通で15,000 ms。10 msごとに右→左→EL05の1台を処理し、各軸は約30 msごとに進む。無応答の軸が他軸の処理を妨げない。

各軸の初期化は以下の順序で進む。

1. STOPを送信し、その送信時の受信件数より新しい、fault=0・mode=0・有限位置・100 ms未満の応答を待つ。停止位置を保持目標として保存する。
2. run_mode=PP、最大速度10 rad/s、加速度1 rad/s²、電流上限10 A、保持位置の順に書き込む。それぞれ送信登録だけで進めず、新しい正常な停止状態のフィードバックを待つ。
3. Enableを送信し、送信前より受信件数が増えた正常なmode=2の応答を待つ。
4. 確認済みの軸も電流上限の再書き込みで応答を要求し、他軸待ちの間の鮮度を維持する。運転中の応答を全3台同時に確認できれば完了。

HAL送信登録失敗は同じ段階を再試行する。応答待ちのSTOP・設定書き込み・電流上限プローブは50 ms以上の間隔（ラウンドロビンにより通常60 ms）で再送する。Enable後300 ms経っても稼働確認できない場合は、STOPから設定をやり直す。

古い応答、fault、不適切なモードでは確認済みの軸もSTOP待ちに戻る。faultは自動クリアせず、期限内に新しい正常停止応答が得られれば再開する。全台がそろわないまま15秒経過した場合は失敗とする。待機中は上位の移動目標を送らず、初期化時の停止位置を保持させる。

type-18書き込みがtype-2応答を返す形式は、[メーカーRS03マニュアル](https://www.robstride.com/assets/product_manual_robStride03-9c54db7a.pdf)§4.1.9（PDF 22ページ）による。type-2応答は動作状態を示し、書き込んだパラメーターの値やインデックスを含まない。応答待ちで無応答・送信脱落を検出するが、各パラメーターの値そのものの読戻し検証ではない。
...
| 3 | EL05のPOSITION_REF書き込み |
| 4〜8 | 通常のモーター指令なし |
| 9 | カウンターを0へ戻し、上位へ`0x210`を送信 |

周期制御は`motors_running`がtrueの場合だけ送信する。全軸停止処理では先にこのフラグをfalseにし、TIM6を停止するため、STOP処理中に周期指令で上書きしない。
...
| 条件 | 現行の動作 |
| --- | --- |
| 運転開始後、既知4台のいずれからも3,000 ms以上フィードバックなし | 全軸STOPを試行し、CAN診断出力後`NVIC_SystemReset()` |
| 初期化中、RobStrideの1台が無応答 | 他軸も応答確認を続け、15秒以内に全台そろわなければ全軸停止処理 |
| 運転開始後、1台だけ無応答、別の既知モーターから受信継続 | 共通3秒監視は発動しない |
...
初期化失敗経路は`Error_Handler`に入る前に全4台へSTOPを送る。まず運転フラグを解除してTIM6を停止し、CyberGear→右RS03→左RS03→EL05の順に10 ms間隔で送信する。1台の送信失敗で他軸を省略しない。
全軸への送信登録と新しい停止状態(mode=0)の応答が確認できるまで、最大1秒間繰り返す。停止応答はfault付きでも受理する。確認できない場合は`queued`・`stopped`のビットマスク（bit0=CyberGear、bit1=右、bit2=左、bit3=EL05）を出力する。通信不能時の物理的停止は保証できない。
`Error_Handler`自体は生成コードのまま割り込み禁止で無限待ちする。初期化の再試行は期限内だけで、致命的エラー停止後はMCUリセットが必要。

メイン処理と割り込みの共有状態を読む主要箇所ではPRIMASKを保存し、割り込み禁止でスナップショットを取り、元のPRIMASKに戻す。
RobStrideの送信FIFO操作もこの方式で直列化する。初期化の応答スナップショットは3台と現在時刻を同じ割り込み禁止区間で取得する。
...
- 原点探索の各失敗箇所、エッジ測定結果、初期化時間、RobStride初期化期限切れ時のID・段階、停止未確認の軸、復帰待機終了を出力する。
...
2. RS03/EL05のソフトウェア位置範囲制限、上位からの非有限目標の拒否、通常運転時の各台独立の応答監視・fault時の全台停止はない。起動時は全3台の応答とfaultを検査する。
...
7. RobStrideは全3台の稼働状態を確認するが、各パラメーターの読戻しや目標位置到達は確認しない。Enable前のパラメーター・保持位置設定が搭載ファームウェアで保持されることは実機で確認する。
8. 通常運転の3秒監視は全モーター共通であり、RobStride1台の断線を別モーターの応答が隠し得る。
...
- `0x500`初回受信で動かず、`0 → 1`だけで起動し、同値繰り返しや`1 → 0`、`0 → 2`、`2 → 1`で起動しないこと。
...
- 初期化中のRobStride1台無応答・遅延起動・一時送信失敗と、全台無応答・CyberGear単独無応答の挙動。STOPが届かない条件でのモーター側挙動。
- 全RobStrideの初期化完了までタイマーが始まらず、停止位置を保持すること。各設定の応答とEnable前の設定値の保持、ADRCの電流・変化率制限、振動・位置追従性。
