...
周期制御開始後、5 msごとに標準IDのCAN FD・16バイト・BRS OFFで送信する。
...
TIM6は1 msごとに0〜4のフェーズを巡回する。各軸への指令は5 ms周期（200 Hz）で、送信負荷を分散する。
`MOTOR_CONTROL_PHASE_COUNT=5`で設定し、4軸分の送信枠を確保するため4未満はコンパイルエラーとする。
上位への`0x210`返信も5 ms周期（`BOARD_FEEDBACK_INTERVAL_MS=5`）とし、独立したカウンターで設定する。
...
| 4 | 通常のモーター指令なし。次回のフェーズを0に戻す |

`0x210`はTIM6開始後5回目の割り込みから5回ごとに送信する。
RobStrideのPPモードと速度・加速度・電流上限は維持し、位置指令の送信頻度を上げる。
CyberGear ADRCの呼び出しも5 ms周期になるが、状態更新は新規フィードバックを受信したときだけ行う。
原点探索、起動設定間の待機、メインループのポーリング間隔は変更しない。
...
1. `robstride_set_zero()`は原点設定専用ブランチでtype=0x06・Byte0=1に修正済み。専用アプリはSTOP後に送信し、ゼロ付近の新規応答を待ってからCSPで0°保持する。通常の4軸起動処理からは呼ばない。
2. RS03/EL05のソフトウェア位置範囲制限、非有限目標の拒否、各台独立の応答監視、fault時の全台停止はない。
3. 通常の4軸制御ではPC0/PC1のリミット割り込みは制御に結び付いていない。`RS03_HOMING_ONLY=ON`（このブランチの既定値）ではPC0をrightの原点入力としてポーリングし、leftへの制御指令は送らない。専用アプリの起動・保持動作は[README](../README.md)を参照。
...
- 1 msの送信分散、各軸5 ms更新、`0x210`の5 ms周期とCAN負荷、送信FIFO登録失敗の有無。
